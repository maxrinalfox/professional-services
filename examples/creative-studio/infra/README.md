...
In your `dev.tfvars`:
```hcl
frontend_secrets_additional = ["OAUTH_CLIENT_ID"]
backend_secrets = ["OAUTH_CLIENT_ID"]
backend_runtime_secrets = {
  "GOOGLE_TOKEN_AUDIENCE" = "OAUTH_CLIENT_ID"  # Maps env var to secret
}
```

**Important Security Notes:**
- `GOOGLE_TOKEN_AUDIENCE` is NOT stored in `be_env_vars` (it's a secret)
- It's managed via `backend_runtime_secrets` which pulls from Secret Manager at runtime
- This prevents secrets from being exposed in version control
- The database password is auto-generated by Terraform (no manual setup needed)
...
- [ ] `terraform.auto.tfvars` (or `ENV.tfvars`) created and filled:
  ...
  - [ ] `backend_custom_audiences` = [GOOGLE_CLIENT_ID] (if using OAuth)
