...
⚠️ **Note:** Currently, OAuth Client ID must be manually created. Future Phase 3 will consolidate OAuth configuration and automate this process.

**Important Security Note:**
- `GOOGLE_TOKEN_AUDIENCE` is NOT stored in `be_env_vars` (it's a secret)
- It's managed via `backend_runtime_secrets` which pulls from Google Cloud Secret Manager at runtime
